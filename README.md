...
- 构建mysql数据库
- 前将后端部署到服务器
- 修改了前后端交互的方式，引入gunicorn进行交互

---
2025.5.11
- gunicorn部署成功，支持多用同时登录的并行检测
- 进行flask加密，实现多用户登录的密匙检测，去除config的本地检测
- 修改了mysql数据库的错误实现

---
2025.6.11
- 实现MCP的小demo
- 在回答函数部分，我们构建了异步工作的实现逻辑
   
---
2025.6.12
- 实现因果库，可以进行简单的因果pc算法分析
- 实现mcp连通因果库，通过mcp的检测，先可以让llm主动选择调用还是不调用库
- 实现调用vis-network库，将图表数据（节点和边）在这个区域内渲染成一个功能齐全、可供用户交互的动态图表。
- 增加llm上下文阅读功能，现在LLm已经有记忆了，目前最高支持20条历史记录
- 增加了代码中创建了一个在后台线程中持续运行的 asyncio 事件循环。
- 新增历史记录支持保存因果图功能
- 增加应用端多用户并行登录逻辑
- 增加前端动画效果，加载动画
---
...
---
2025.6.15
- bug修改：前端。ai回复的时候加载点一直存在
  - 当用户返回消息的时候，ai会回复已上传成功
- 当用户同时上传多个文件，MCP无法响应 
