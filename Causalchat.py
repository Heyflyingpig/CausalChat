...
SECRETS_PATH = os.path.join(BASE_DIR, "secrets.json") # <-- 新增：敏感信息配置文件路径

current_logged_in_user = None # <-- 新增：全局变量

# --- 修改：从环境变量或 secrets.json 加载 API 配置 ---
default_api = "zhipuai"
default_model = "glm-4-flash"
default_base_url = "https://open.bigmodel.cn/api/paas/v4/"
default_apikey = "YOUR_DEFAULT_FALLBACK_API_KEY_IF_ANY" # 最好不要有默认的真实密钥

# 全局变量，将由 load_api_config 初始化
    ...
    try:
        if os.path.exists(SECRETS_PATH):
            with open(SECRETS_PATH, "r", encoding="utf-8") as f:
                ...
                apikey = secrets_data.get("ZHIPUAI_API_KEY", default_apikey)
                BASE_URL = secrets_data.get("ZHIPUAI_BASE_URL", default_base_url)
                current_model = secrets_data.get("ZHIPUAI_MODEL", default_model)
                # current_api 可以根据需要从 secrets.json 读取或保持默认
                logging.info(f"API 配置已从 {SECRETS_PATH} 加载。")
        else:
            logging.warning(f"敏感信息配置文件 {SECRETS_PATH} 不存在。将尝试使用环境变量或默认值。")
            # 如果文件不存在，我们已经尝试了环境变量，所以这里会依赖上面设置的默认值
    except FileNotFoundError: #  <-- 新增
        logging.warning(f"敏感信息配置文件 {SECRETS_PATH} 未找到。") #  <-- 新增
    except json.JSONDecodeError: #  <-- 新增
        logging.error(f"解析敏感信息配置文件 {SECRETS_PATH} 失败。请检查文件格式。") #  <-- 新增
    except Exception as e: #  <-- 新增 (捕获其他未知错误)
        logging.error(f"加载 API 配置时发生未知错误: {e}") #  <-- 新增
